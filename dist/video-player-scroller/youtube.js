  ...
  const scrollerOptions = {
    ...
    setupSubtitles(player) {
      ...
      function isAutoGeneratedTrack(track) {
        return (
          (track.kind || '').toLowerCase() === 'asr' ||
          (track.vss_id || '').toLowerCase().startsWith('a.') ||
          (track.displayName || '').toLowerCase().includes('auto-generated') ||
          (track.languageName || '').toLowerCase().includes('auto-generated')
        );
      }

      if (player.getOption && player.setOption && player.isSubtitlesOn && player.toggleSubtitles) {
        const isSubtitlesOn = player.isSubtitlesOn();
        const currSubs = player.getOption('captions', 'track');
        if (isSubtitlesOn && currSubs) {
          if (!isEnglishTrack(currSubs)) {
            player.toggleSubtitles();
          }
          if (isAutoGeneratedTrack(currSubs)) {
            player.toggleSubtitles();
          }
        }

        const subs = player.getOption('captions', 'tracklist', { includeAsr: true }) || [];
        const engManualSubs = subs.find((track) => isEnglishTrack(track) && !isAutoGeneratedTrack(track));
        if (engManualSubs) {
          player.setOption('captions', 'track', engManualSubs);
        }
      }
    },
  };
